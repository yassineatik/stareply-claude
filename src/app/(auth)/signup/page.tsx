...
function SignUpForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Save pending plan if specified in URL
    ...
    <>
      <div className="text-center mb-6">
        <h2 className="font-display text-2xl font-bold text-white">
          Start replying for free
        </h2>
        <p className="mt-2 text-sm text-void-100 font-body">
          No credit card. Setup in 90 seconds.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div>
          <input
            ...
            required
            autoComplete="name"
            className="w-full rounded-[var(--radius-md)] bg-white/[0.05] border border-white/[0.10] px-3.5 py-2.5 text-[15px] text-white placeholder:text-void-400 font-body focus:border-violet-500 focus:ring-2 focus:ring-violet-500/30 focus:outline-none transition-all"
          />
        </div>
